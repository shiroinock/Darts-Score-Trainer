...
TODO.mdの次タスク「基礎練習モードのランダム出題化（82ターゲット対応）」の2番目のサブタスク「PracticeConfig型にrandomizeTarget: booleanフラグを追加（オプショナル、デフォルトfalse）」について、実装対象ファイルのテストパターンを判定してください。\n\n対象ファイル: src/types/PracticeConfig.ts\n\n以下を出力してください:\n1. tddMode (test-first / test-later)\n2. testPattern (unit / store / hook / component / integration)\n3. placement (colocated / separated)\n4. rationale (判定理由)\n5. testFilePath (テストファイルの配置パス)
```
