...
3. 出力されるマトリクスを保存：
   ```json
   {
     ...
     "matrix": {
       ...
       "coordinates": ["file1.ts"],
       "specification": ["file3.ts"]
     },
     "uncovered_files": [...],
     "suggested_aspects": [...]
   }
...
1. マトリクスの各組み合わせに対して `review-file` サブエージェントを**並列起動**
   ...
   レビュー観点: .claude/review-points/coordinates.md
   ```
2. 各レビュー結果を収集
3. 結果をマトリクス形式で整理：
   ```
...
|----------|------------|--------|------|
| src/utils/coordinateTransform.ts | PASS | PASS | - |
...
- **提案チェック項目**:
  - [ ] 必須フィールドが設定されているか
  - [ ] パスの整合性が取れているか
  - [ ] GitHub Pages用のbase設定が正しいか

## 修正履歴

### ループ 1
**検出された問題**:
- src/utils/coordinateTransform.ts:42 [coordinates] 変数名が不明確
