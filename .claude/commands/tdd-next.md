...
### 5. TODO.md更新とDONE.mdへの記録

タスク完了後、TODO.mdとDONE.mdを更新します:

#### 5.1 完了タスクのDONE.mdへの移動
1. 完了したタスクをDONE.mdの末尾（`---`の前）に追記
   - セクションヘッダー（`### x.x ...`）を含める
   - タスク行は `- [x]` 形式で記録
   - 実装したファイルパス、日付を含める（推奨）

2. DONE.mdの最終更新日を更新
   ```markdown
   ### x.x セクション名
   - [x] タスク名 (実装: src/path/to/file.ts, テスト: src/path/to/file.test.ts) - 2025-01-04
...
#### 5.2 TODO.mdからの削除
- 完了したタスク行をTODO.mdから削除
- セクションに未完了タスクが残っていない場合、セクションヘッダーも削除
- Phaseに未完了タスクが残っていない場合、Phaseヘッダーも削除
...
- 完了したタスクはDONE.mdに移動し、TODO.mdから削除してください（セクション5参照）
...
9. TODO.mdとDONE.mdが更新される（完了タスクをDONE.mdに移動、TODO.mdから削除）
