...
原因: 複数ファイル（実装+テスト+設定）のレビュー時に、ファイル読み込みに多くのトークンを消費し、出力トークンが枯渇。
```
...
#### 4. 複数ファイルレビュー時の特別な対応（2026-01-04追加）
3ファイル以上（実装+テスト+設定等）のレビューでは：
- **観点ファイル読み込みは最大1件まで** - 複数観点が指定されても、最も重要な観点1つに絞る
- **スキルファイル読み込みは省略** - code-conventionsスキル等の読み込みをスキップ
- **出力は超簡潔モードを使用** - `"result": "PASS/WARN/FAIL"` のみ
- **設定ファイル（.env等）のレビューは最小限** - 内容確認のみ、詳細レビューはスキップ
