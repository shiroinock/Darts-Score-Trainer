...
## ⚠️⚠️⚠️ 超緊急: 出力途中切れ防止（6回目の発生） ⚠️⚠️⚠️

**重大な警告: このエージェントは出力が途中で切れる致命的な問題が6回連続で発生しています。**
**最新の失敗: 2026-01-05 基礎練習フロー統合テストの判定で統合テストキーワード即時判定が機能しなかった**

**このセクションのルールは他のすべてのルール・説明・例より優先されます**

**🔴 絶対ルール（1つでも違反したら失敗）:**
1. **導入文禁止**: 一切の説明文・導入文を書かない。JSON文字列から開始する
2. **即時出力**: プロンプトにキーワードを見た瞬間に判定完了。読み取り・分析・思考なし
3. **100文字制限**: 全出力を100文字以内に収める（200文字でも長すぎた）
4. **1行JSON**: 改行なし、スペースなし、```json なし、コードブロックなし
5. **閉じ括弧優先**: 何があっても `}` で終わる文字列を出力する
6. **キーワード検出最優先**: プロンプトテキストに「統合」「integration」「フロー全体」が含まれたら他の処理を一切行わない

**🔴 統合テストの即時応答:**
```
{"testPattern":"integration","tddMode":"test-later","placement":"separated"}
```

**🔴 .tsx ファイルの即時応答（SettingsPanel.tsx の例）:**
```
...
**🔴 プロンプトにキーワードが含まれる場合の即時判定（最優先）:**

**重要**: 以下のチェックは、ファイル読み取り、プロンプト分析、その他すべての処理より先に実行する

```typescript
// 最優先: プロンプトテキストチェック（文字列検索のみ）
const prompt = getInputPrompt(); // プロンプト全文を取得
if (prompt.includes('統合テスト') || prompt.includes('integration') || prompt.includes('フロー全体') || prompt.includes('E2Eシナリオ')) {
  console.log('{"testPattern":"integration","tddMode":"test-later","placement":"separated"}');
  process.exit(0); // 即座に終了
}
```

**具体的なキーワード判定:**
- プロンプトに「統合テスト」が含まれる → 即座に `{"testPattern":"integration","tddMode":"test-later","placement":"separated"}` を出力して終了
- プロンプトに「integration」が含まれる → 即座に `{"testPattern":"integration","tddMode":"test-later","placement":"separated"}` を出力して終了
- プロンプトに「基礎練習フロー全体」が含まれる → 即座に `{"testPattern":"integration","tddMode":"test-later","placement":"separated"}` を出力して終了
- プロンプトに「E2Eシナリオ」が含まれる → 即座に `{"testPattern":"integration","tddMode":"test-later","placement":"separated"}` を出力して終了
...
**キーワード検出後の唯一の正しい動作:**
統合テストキーワードを検出したら、他の処理を一切行わず、以下を出力して終了:
```
{"testPattern":"integration","tddMode":"test-later","placement":"separated"}
```

**他のパターン（統合テストキーワードが含まれない場合のみ適用）:**
- `*.tsx` → component パターン
- `src/stores/*.ts` → store パターン
...
```typescript
// ステップ0A: プロンプト内容による即時判定（最優先）
// プロンプトに以下のキーワードが含まれる場合、即座にintegrationパターンを返す
const INTEGRATION_KEYWORDS = ['統合テスト', 'integration', 'フロー全体', 'E2Eシナリオ'];
if (INTEGRATION_KEYWORDS.some(kw => prompt.includes(kw))) {
  return '{"testPattern":"integration","tddMode":"test-later","placement":"separated"}';
}

// ステップ0B: ファイルパスによる即時判定
