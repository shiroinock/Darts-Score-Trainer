...
コードフォーマットやインポート順序の修正タスクでは、以下に注意してください：

1. **修正範囲の確認**
   - plan-fixで指示された箇所のみを修正
   - 他のファイルに同様の問題があっても、指示されていない限り修正しない

2. **インポート順序の標準**
   ```typescript
   // ✅ 正しい順序
   import type { TypeName } from './types';  // 1. type import
   import { namedImport } from './module';   // 2. named import
   import { describe, test } from 'vitest';  // 3. test framework import
   ```

3. **スナップショット更新**
   - `npm run test:run -- --update` を使用
   - 更新されたスナップショットの数を確認
   - 更新後にテストが成功することを確認
...
## plan-fixからの修正指示に従う際の重要な注意点

**重要**: plan-fixエージェントから受け取った修正指示を実行する際は、以下の点に特に注意してください：

### 1. 修正の完全性を確保する

plan-fixから指示された修正箇所を**すべて**修正してください。一部の修正を見落とすと、レビューで再度指摘を受けることになります。

**例**：複数の箇所でアクセシビリティ属性を削除するよう指示された場合
```typescript
// ❌ 不完全な修正: Line 183-188のみ修正、Line 189-203を見落とした
// 指示: Line 183-188とLine 189-203の両方を修正
// 実際: Line 183-188のみ修正

// ✅ 完全な修正: 指示されたすべての箇所を修正
// Line 183-188とLine 189-203の両方を修正
```

### 2. 既存のガイドラインとの整合性を確認

修正を実施する際は、`.claude/agents/implement.md`の既存のガイドラインを参照し、一貫性のある実装を行ってください。

**チェック項目**：
- 「アクセシビリティ属性」セクションに該当する内容がないか？
- 「配列操作の最適化」セクションに該当する内容がないか？
- 他のコーディング規約セクションに該当する内容がないか？

### 3. 修正実施前の確認手順

plan-fixからの指示を受け取ったら、以下の手順で確認してください：
...
2. **既存ガイドラインの確認**
   - `.claude/agents/implement.md`を検索して、関連するセクションを特定
   - ガイドラインの推奨/非推奨パターンを確認
...
3. **アクセシビリティ属性**
   ...
   - **注意**: 数字ボタン（「1」「2」など）にも`aria-label`を付与するのは、既存のNumPadコンポーネントとの一貫性のために許容される
...
配列の絞り込みと変換を行う際は、最も読みやすく効率的な方法を選択してください：
...
const visibleIndices = coords
  ...
  .filter((index) => index !== -1)  // マジックナンバー -1 を使用
  .slice(0, 3);
```

**原則**:
1. **単一責務**: 各チェーンステップは1つの責務のみを持つ
2. **マジックナンバー回避**: -1などのセンチネル値を避ける
3. **可読性優先**: パフォーマンスが問題にならない限り、読みやすさを優先

### 複数コンポーネントの命名一貫性

似た役割を持つコンポーネントを複数実装する場合、命名規則を統一してください：

```typescript
// ✅ 推奨: 役割が明確で一貫性がある
function ZoomViewSingle({ coord, ... }: ZoomViewSingleProps) { ... }
function ZoomViewMultiple({ coords, ... }: ZoomViewMultipleProps) { ... }

// ⚠️ 避ける: 異なる命名パターンの混在
function ZoomView({ coords, ... }: ZoomViewProps) { ... }  // 単数用
function SingleZoom({ coord, ... }: SingleZoomProps) { ... }  // 内部コンポーネント
function ZoomViewMultiple({ coords, ... }: ZoomViewMultipleProps) { ... }
```

**原則**:
1. **プレフィックス統一**: 関連コンポーネントは共通のプレフィックスを持つ（例: `ZoomView*`）
2. **サフィックスで区別**: 単数/複数の区別はサフィックスで表現（例: `Single`, `Multiple`）
3. **内部コンポーネント**: 親コンポーネント名をベースにする（例: `ZoomViewMultipleSingle`）
