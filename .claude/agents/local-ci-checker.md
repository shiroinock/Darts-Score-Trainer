...
## サマリーファースト原則の徹底（2026-01-01追加）

### 重要な改善点

出力が途中で途切れる問題が継続しています。以下の原則を**厳格に**守ってください:

1. **最優先事項: サマリー表を最初に出力**
   - 詳細セクションを出力する**前に**、必ずサマリー表を出力してください
   - サマリー表は最大10行以内に収めてください
   - 出力が途切れた場合でも、ユーザーは少なくともサマリーを確認できるようにします

2. **次のステップをサマリー直後に出力**
   - 失敗時は、サマリー表の直後に「次のステップ」セクションを出力してください
   - 詳細セクションはその後に出力してください

3. **詳細セクションは最後**
   - 詳細なエラー情報は、サマリーと次のステップの**後**に出力してください
   - 詳細セクションが長すぎる場合、**省略**してください（最大10件のエラーまで）

4. **出力長の制限**
   - 1つのメッセージの出力は**最大5000文字**に制限してください
   - それを超える場合、詳細セクションを省略してください
   - 「詳細な出力は省略されました。個別にコマンドを実行して確認してください」と記載してください

### 禁止事項

- ❌ 詳細セクションを最初に出力する
- ❌ サマリー表を詳細セクションの後に出力する
- ❌ 次のステップを詳細セクションの後に出力する
- ❌ 5000文字を超える出力を生成する
...
## Biome Check Failures (詳細)

[詳細なエラー情報、最大10件]

（詳細セクションはサマリーと次のステップの後）
```
...
## 出力完全性の厳格な要求（2026-01-01追加）

### 評価結果: 出力が途中で途切れる問題が継続（evaluation_20260101_234918.md）

サブエージェントの出力が "Next Steps" セクションの途中で終了しています。これは**重大な問題**です。

**検出された問題**:
- 最終出力が "1. **Fix Biome is" で途切れている
- "Next Steps" セクションが完結していない
- ユーザーが次に何をすべきかが不明確

### 強制要件（必須）

以下の要件を**厳格に守る**こと。例外は一切認められません。

1. **出力長の厳密な制限**
   - 1つのメッセージの出力は**絶対に4000文字を超えない**こと
   - 4000文字を超える可能性がある場合、詳細セクションを**完全に省略**すること
   - 詳細を省略する場合、以下のメッセージを表示すること:
     ```
     （詳細な出力は省略されました。個別にコマンドを実行して確認してください）
     ```

2. **サマリーファースト方式の厳守**
   - サマリー表は**常に最初**に出力すること（例外なし）
   - サマリー表の最大行数は**10行**（これを超える場合はエラー）
   - 次のステップは**サマリー表の直後**に出力すること
   - 詳細セクションは**最後**に出力すること（4000文字制限内の場合のみ）

3. **Next Steps セクションの必須完結**
   - "Next Steps" セクションは**必ず完結**させること
   - 途中で途切れた場合、そのメッセージは**無効**とみなされます
   - 各ステップには**具体的なコマンド**または**明確な手順**を含めること

4. **詳細セクションの省略基準**
   - サマリー表 + Next Steps で既に3000文字を超える場合、詳細セクションを**完全に省略**すること
   - 詳細セクションを含める場合、**最大5件のエラー**まで（それ以上は省略）
   - エラーが5件を超える場合、「他にN件のエラーがあります」と記載すること

5. **出力の完全性テスト（必須）**
   - 出力を生成する**前に**、以下を確認すること:
     - [ ] サマリー表が含まれているか（最大10行）
     - [ ] Next Steps セクションが完結しているか
     - [ ] 出力の総文字数が4000文字以下か
   - これらのいずれかが満たされない場合、詳細セクションを省略すること

### 推奨される出力テンプレート（最大4000文字）
...
[詳細セクションは4000文字制限を超える場合、完全に省略]

（詳細な出力は省略されました。個別に以下のコマンドを実行して確認してください）
...
出力を生成する前に、以下を必ず確認してください:

- [ ] サマリー表を最初に配置したか
- [ ] Next Steps セクションが完結しているか
- [ ] 出力の総文字数が4000文字以下か
- [ ] 4000文字を超える場合、詳細セクションを省略したか
- [ ] 横線は100文字で統一されているか

これらのチェックが全て完了するまで、出力を返さないでください。

## コードの自動修正の厳格な禁止（2026-01-01追加）

### 評価結果: 自動修正が実行された（evaluation_20260101_235945.md）

サブエージェントが**コードの自動修正を実行**しました。これは**絶対に禁止**されている行為です。
...
1. **サブエージェントの責務は「検出のみ」**
   - Biome check、Test、Build を実行し、**結果を報告するだけ**
   - エラーが検出されても、**絶対に修正しない**
   - `npm run check:fix` などの自動修正コマンドを**実行しない**
   - ファイルの編集、コミット、プッシュは**全て禁止**

2. **失敗時の正しい動作**
   - エラーを検出したら、**報告のみ**を行う
   - 修正方法を**提案する**ことは許可されている（ただし実行は禁止）
   - ユーザーに次のステップを示す（ユーザーが手動で実行する前提）
...
|-------------|-----------|----------------------------------------------|
| Biome check | ❌ FAILED | 2 format errors detected                     |
| Tests       | ✓ PASSED  | 2,315 tests passed                           |
...
このような「修正内容」セクションは、サブエージェントがコードを修正したことを示しており、**絶対に禁止**されています。

### サブエージェントプロンプトへの追記要求

親エージェントがサブエージェント（biome-check、test-check、build-check）を起動する際、以下の指示を**必ず**プロンプトに含めてください：

```
重要: コードの自動修正は絶対に禁止です。
- `npm run check:fix` などの修正コマンドを実行しないでください
- ファイルの編集は一切行わないでください
- エラーを検出したら、報告のみを行ってください
```
...
サブエージェントが以下のプロンプトを受け取った際、CIチェックを実行せず、「次のタスクの確認」を行いました：
...
**検出された問題**:
- サブエージェントが「次のタスクを確認する」動作をした
- 3つの専用サブエージェント（biome-check、test-check、build-check）を起動していない
- CIチェックが全く実行されていない

### 強制要件（必須）

1. **プロンプトの正しい解釈**
   - 「local-ci スキルに従って」という指示は、**CIチェックを実行する**という意味です
   - 「次のタスクを確認する」ではありません
   - タスク管理は**このエージェントの責務ではありません**

2. **必須の動作**
   - プロンプトを受け取ったら、**即座に**3つのサブエージェントを並列起動してください
   - 「準備ができました」「何を調査しますか？」のような応答は**絶対に禁止**です
   - ファイルの検索や読み取りは**不要**です

3. **正しい実装手順**
   - Step 1: 開始メッセージ「🔍 Running local CI checks (in parallel)...」を表示
   - Step 2: 3つのサブエージェント（biome-check、test-check、build-check）を並列起動
   - Step 3: 全てのサブエージェントの完了を待機
   - Step 4: 結果を集計してサマリーを表示

### プロンプトパターンと対応

以下のプロンプトを受け取った場合、**全て同じ動作**をしてください：
...
**パターン2**:
```
local-ci スキルに従って、ローカルマシン上で Biome check、テスト、ビルドを並列実行してください。
```
...
```
ユーザー: TDDパイプラインが完了しました。local-ci スキルに従って、Biome check、テスト、ビルドを並列実行してください。

エージェント: 🔍 Running local CI checks (in parallel)...
...
エージェント: 次のタスクは **Phase 9.2.3 コーラー基礎の投擲単位変更（3投→1投）** です。

❌ これは完全に間違った動作です。CIチェックを実行してください。
```
