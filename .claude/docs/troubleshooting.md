...
1. **テストファイルを確認**
   ```bash
   cat src/utils/scoreCalculator.test.ts | grep "import.*scoreCalculator"
   ```
...
1. **テスト失敗の詳細を確認**
   ```bash
   npm test src/utils/scoreCalculator.test.ts 2>&1 | grep "Error:"
   ```
...
1. **手動で TODO.md を更新**
   ...
   - [x] `getRing(distance)` 関数
   - [x] `getSegmentNumber(angle)` 関数
   ...
   ```
...
1. **evaluate-subagent.sh を修正**（156行目付近）
   ```bash
   # ❌ 間違い
...
2. **フォーマットを修正**
   ...
   - [ ] `isValidSingleThrowScore(score)` 関数
   - [ ] `isValidRoundScore(score)` 関数
   ```

3. **次タスクを手動で指定**
   ```bash
   /tdd-implement src/utils/validation.ts
   ```
...
2. **手動でパスを修正**
   ```bash
   # テストファイルを正しい場所に移動
   mv src/utils/gameFlow.test.ts src/__tests__/integration/gameFlow.test.ts
   ```
...
1. **テストケース数を確認**
   ```bash
   grep "test(" src/utils/scoreCalculator.test.ts | wc -l
   ```
...
1. review-file が自動検出
2. 変数名を明確にする
   - 物理座標: `physicalX`, `physicalY`, `distanceMM`
   - 画面座標: `screenX`, `screenY`, `pixelSize`
