...
```bash
# TODO.md の次タスク「1.3 座標変換」を実装
/tdd-next

# 結果:
# - coordinateTransform.ts が作成される
# - coordinateTransform.test.ts が作成される
# - 全テストが Green になる
# - TODO.md が更新される
```
...
- **colocated**: 同階層
  - 例: `src/utils/scoreCalculator.test.ts`
- **separated**: `src/__tests__/integration/`
  - 例: `src/__tests__/integration/gameFlow.test.ts`
...
- [評価プロンプト](.claude/evaluation-prompts/)
